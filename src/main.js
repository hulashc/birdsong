...
function energyToColor(e, tint = 'primary') {
  ...
  if (tint === 'secondary') {
    c.setRGB(0.55 + t * 0.40, 0.42 + t * 0.38, 0.10 + t * 0.05);
  } else {
    if (t < 0.5) {
      const u = t / 0.5;
      c.setRGB(0.18 + u * 0.52, 0.20 + u * 0.55, 0.35 + u * 0.45);
    } else {
      const u = (t - 0.5) / 0.5;
      c.setRGB(0.70 + u * 0.22, 0.75 + u * 0.16, 0.80 - u * 0.16);
    }
  }
  return c;
}
...
function addAxisLabel(text, pos, col) {
  ...
  el.textContent = text;
  el.style.cssText = [
    `color:${col}`, 'font-size:8px', 'letter-spacing:.2em',
    'font-weight:700', 'text-transform:uppercase',
    'font-family:Helvetica Neue,sans-serif', 'opacity:0.45', 'padding:1px 4px',
  ].join(';');
...
function inkPath(slot, i, e) {
  ...
  const c   = energyToColor(e, tint);
  inkPos[slot.inkHead*3]   = pos.x; inkPos[slot.inkHead*3+1] = pos.y; inkPos[slot.inkHead*3+2] = pos.z;
  inkCol[slot.inkHead*3]   = c.r * 0.7; inkCol[slot.inkHead*3+1] = c.g * 0.7; inkCol[slot.inkHead*3+2] = c.b * 0.7;
  slot.inkHead++;
